...
pub fn get_meta(buffer: & [u8]) -> (bool, bool, u8, u16, u16, u32, u32, String){
    ...
    let name = byte_to_string(&mut &buffer[13..77]);
...
pub fn wrapper_bool(buffer: &Vec<u8>) -> Vec<PackCompact>{
    // total length 73 + len
    let (kind, bin, method, relative, thresh, length, bytes, name) = get_meta(buffer);
    ...
    if matches.is_present("pack") | (matches.is_present("index") & matches.is_present("compressed pack")) {
        ...
        //READ COVERAGE AND META
            ...
            if Path::new(matches.value_of("index").unwrap()).exists() & Path::new(matches.value_of("compressed pack").unwrap()).exists() {
                p = wrapper_compressed(matches.value_of("index").unwrap(), matches.value_of("compressed pack").unwrap());
            } else {
                no_file = true;
            }
        }
...
impl PackCompact {
    ...
    pub fn read_bin_coverage(buffer: &[u8]) -> Self {
        ...
        PackCompact { name: name, node: Vec::new(), node_coverage: Vec::new(), bin_coverage: bv, coverage: Vec::new() }
    }
    ...
    pub fn wrapp(file_pc: &str) -> Self{
        ...
        if meta.1{
            let a = Self::read_bin_coverage(&buff);
            return a
        } else {
            let a=  Self::read_u16(&buff);
            return a
        }
    }
    ...
    pub fn read_u16(buffer: &[u8]) -> Self{
        ...
        PackCompact {name: name, node: Vec::new(), node_coverage: Vec::new(), bin_coverage: BitVec::new(), coverage: data}
    }
